...
<body>
  ...
  <p style="margin-top: 3em; font-style: italic; font-size: 0.9em; color: #777;">Được biên dịch & đóng gói tự động bằng VIP EPUB Tools</p>
            ...
            for i, (ch_title, ch_content) in enumerate(self.chapters):
                # If content already looks like HTML/XHTML, we wrap it properly, otherwise build from plain text paragraphs
...
def apply_custom_dictionary(text, custom_dict):
    """
    Replaces Chinese words with user-provided Vietnamese translations
    before running the translation engine to force custom glossary mappings.
    custom_dict should be a list of tuples: (chinese_word, translated_word)
    """
    if not custom_dict or not text:
        return text
    for zh, vi in custom_dict:
        if zh and zh in text:
            # We wrap with a special token or replace directly
            # Directly replacing works fine if it's already translated names
...
def translate_html_content_advanced(html_content, engine, mode=None, custom_dict=None, clean_styles=False):
    ...
    Applies name mapping from custom dictionary and cleans styles if enabled.
    """
    if clean_styles:
...
def translate_epub_file(src_epub, dest_epub, engine, mode=None, limit_chapters=-1, custom_dict=None, clean_styles=False, strip_images=False, strip_fonts=False):
    ...
    Supports chapter limit (limit_chapters > 0) for quick previews.
    Supports stripping images and fonts to optimize for mobile.
    """
        ...
        with zipfile.ZipFile(dest_epub, 'w', compression=zipfile.ZIP_DEFLATED) as dest_zip:
            ...
            for file_name in file_list:
                ...
                if file_name in target_html_files:
                    ...
                    # Keep static resources (images, fonts) intact
                    dest_zip.writestr(file_name, raw_data)
...
def convert_txt_to_epub(txt_content, title, author, split_regex, dest_path, description="", engine=None, mode=None, custom_dict=None):
    """
    Converts raw text file content to EPUB. Splits into chapters using split_regex.
    Optionally translates each chapter and applies custom dictionary.
    """
