...
function parseDict(text) {
    ...
    for (let line of lines) {
        ...
        const right = cleanAnnotation(line.slice(idx + 1).trim());
...
class Trie {
    ...
    insert(word, translation, priority) {
        let node = this.root;
    ...
    searchLongestMatch(text, startIdx) {
        let node = this.root;
        let longestLength = 0;
        ...
        return { length: longestLength, translation: bestTranslation, priority: bestPriority };
    }
}

let vietphraseTrie = new Trie();
let hanvietTrie = new Trie();

function buildTries() {
    vietphraseTrie = new Trie();
    hanvietTrie = new Trie();
    
    if (dictNames) {
        for (let [k, v] of dictNames.entries()) {
            vietphraseTrie.insert(k, v, 1);
            hanvietTrie.insert(k, v, 2);
        }
    }
    if (dictVietphrase) {
        for (let [k, v] of dictVietphrase.entries()) {
            vietphraseTrie.insert(k, v, 2);
        }
    }
}
...
function translateSentence(sentence, mode = 'advanced') {
    ...
    if (!/[\u4e00-\u9fff\u3000-\u303f\uff00-\uffef]/.test(body)) return sentence;
    
    let i = 0;
    const len = body.length;
    const isHanvietOnly = (mode === 'hanviet');
    const isFastTrieMode = (mode === 'hanviet' || mode === 'vietphrase');
    const trie = isHanvietOnly ? hanvietTrie : vietphraseTrie;
    
    if (isFastTrieMode) {
        let resultWords = [];
        while (i < len) {
            const { length, translation, priority } = trie.searchLongestMatch(body, i);
            if (length > 0) {
                const word = body.substring(i, i + length);
                let formatted = formatTranslation(translation, word);
                if (priority === 1 || dictNames.has(word)) {
                    formatted = capitalizePhrase(formatted);
                }
                resultWords.push(formatted);
                i += length;
            } else {
                const char = body[i];
                if (!/[\u4e00-\u9fa5]/.test(char)) {
                    ...
                    resultWords.push(punctMap[char] || char);
                } else {
                    let trans = dictChar && dictChar.has(char) ? dictChar.get(char).split("/")[0] : char;
                    resultWords.push(trans);
                }
                i++;
            }
        }
        
        let translatedText = resultWords.join(" ");
        translatedText = cleanPunctuationSpacing(translatedText);
        ...
        for (let idx = 0; idx < sentences.length; idx++) {
            ...
            if (s && s.trim() && !/^[.!?]/.test(s)) {
                for (let cIdx = 0; cIdx < s.length; cIdx++) {
                    if (/[a-zA-Zà-ỹđĐ]/.test(s[cIdx])) {
                        sentences[idx] = s.substring(0, cIdx) + s[cIdx].toUpperCase() + s.substring(cIdx + 1);
                        break;
                    }
                }
            }
        }
        return leading + sentences.join("").trim() + trailing;
    }
    
    // Standard / Advanced / Fast / Advanced Hanviet modes: Tokenize then reorder
    let tokens = [];
    while (i < len) {
        const char = body[i];
        ...
        // Use Trie to tokenize
        const { length, translation, priority } = trie.searchLongestMatch(body, i);
        if (length > 0) {
            const word = body.substring(i, i + length);
            let trans = formatTranslation(translation, word);
            let type = 'phrase';
    ...
    while (j < tokens.length) {
        ...
        if (isChineseNumber(tok.word) && j + 1 < tokens.length && NUM_KEYWORDS.has(tokens[j+1].word)) {
            ...
            for (let char of combinedWord) {
                if (HANVIET_NUMBERS[char]) translatedParts.push(HANVIET_NUMBERS[char]);
                else if (dictChar && dictChar.has(char)) translatedParts.push(capitalizePhrase(dictChar.get(char).split("/")[0]));
    ...
    let finalTokens = groupedTokens;
    
    // Reordering pass 1: Adjective + Noun (for words ending with 的)
    let changed = true;
    while (changed) {
        changed = false;
        let k = 0;
        while (k < finalTokens.length - 1) {
            let t_a = finalTokens[k];
            let t_n = finalTokens[k+1];
            
            const PREPOSITIONS = new Set(['跟', '和', '与', '與', '同', '在', '从', '從', '自', '由', '向', '往', '朝', '对', '對', '给', '給', '比', '是', '叫', '让', '讓', '被', '把', '使', '令', '到', '了', '的', '而', '&', '并', '並', '以', '或', '者', 'de']);
            if (PREPOSITIONS.has(t_n.word) || t_n.type === 'punctuation' || t_n.type === 'de') {
                k++;
                continue;
            }
            
            if (t_a.word.endsWith('的') && t_a.word !== '的') {
                let combined = t_n.translated + " " + t_a.translated;
                finalTokens.splice(k, 2, { word: t_a.word + t_n.word, translated: combined, type: t_n.type });
                changed = true;
                break;
            }
            k++;
    ...
    // Reordering pass 2: "的" (de) particle reordering
    let k = 1;
    ...
    while (k < finalTokens.length - 1) {
        let tok = finalTokens[k];
        if (tok.word === '的' || tok.word === 'de') {
            let t_x = finalTokens[k-1];
            
            let isVerb = false;
            if (/[过過了着著]/.test(t_x.word)) isVerb = true;
            else {
            ...
            let nextPos = k + 1;
            let y_tokens = [];
            while (nextPos < finalTokens.length) {
                let tok_next = finalTokens[nextPos];
                if (tok_next.type === 'punctuation' || tok_next.word === '的' || tok_next.word === 'de' || 
                    LOCALIZERS.has(tok_next.word) || STOP_WORDS.has(tok_next.word)) {
                    break;
                }
                if (y_tokens.length >= 3) break;
            ...
            if (y_tokens.length > 0 && !isVerb) {
                ...
                let combined = isProperOrPronoun ? (y_translated + " của " + t_x.translated) : (y_translated + " " + t_x.translated);
                
                finalTokens.splice(k-1, nextPos - (k-1), {
                    word: t_x.word + '的' + y_word,
                    translated: combined,
                    type: 'phrase'
                });
                k -= 1; // re-evaluate from the newly merged token
            } else {
                tok.translated = "";
            }
        }
    ...
    let result = finalTokens.map(t => t.translated).filter(Boolean);
    let output = result.join(" ");
    output = cleanPunctuationSpacing(output);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "FETCH_TRANSLATE") {
        ...
        const { token, user, serverUrl, apiKey } = request.payload;
        if (token && user) {
            chrome.storage.local.get(['serverUrl', 'settings'], (res) => {
                const sUrl = serverUrl || res.serverUrl || "";
                const settings = res.settings || {};
                settings.apiHost = sUrl; // Keep apiHost and serverUrl in sync
                if (apiKey) {
                    settings.vipKey = apiKey;
                }
                ...
                if (user.vip_status === 1) {
                    if (settings.membershipType !== 'vip') {
                        ...
                        settings.vipKey = settings.vipKey || apiKey || 'VIP_SERVER';
                        changed = true;
                    }
                }
                if (changed || apiKey) {
