...
function extractUsingTreeWalker() {
    ...
    while (stack.length > 0) {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            if (translatedNodes.has(node) || node.__is_typing) continue;
            ...
            if (/[\u4e00-\u9fa5]/.test(val)) {
                ...
                // If this text chunk was already translated this session, apply instantly and skip network
                if (translationCache.has(cleanVal)) {
                    node.__original_chinese__ = val;
                    node.nodeValue = translationCache.get(cleanVal);
...
function applyChunk(node, translation) {
    ...
    if (node.__original_chinese__) {
        const cleanOriginal = node.__original_chinese__.replace(/[\u200B-\u200D\uFEFF]/g, '');
        translationCache.set(cleanOriginal, translation);
    }
...
// ============================================================
if (location.hostname === 'localhost' || location.hostname.endsWith('lyvuha.com') || location.hostname.endsWith('hf.space')) {
    const checkAndSyncAuth = () => {
        try {
            const token = localStorage.getItem('accessToken');
            const userStr = localStorage.getItem('user');
            const apiKey = localStorage.getItem('local_tts_key') || '';
            if (token && userStr) {
                chrome.runtime.sendMessage({
    ...
    window.addEventListener('sync-auth-event', checkAndSyncAuth);
}
